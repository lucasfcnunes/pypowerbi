...
class Imports:
    ...
    def upload_file(
        self, filename, dataset_displayname, nameconflict=None, group_id=None
    ):
        if group_id is None:
            groups_part = "/"
        else:
        ...
        prepared_displayname = re.sub(
            self.upload_file_replace_regex, "-", dataset_displayname
        )
        ...
        url = (
            f"{self.base_url}{groups_part}{self.imports_snippet}"
        ...
        # 490 Conflict (due to name)
            ...
            raise HTTPError(
                response, f"Upload file failed with status code: {response.json()}"
            )
    ...
    def get_import(self, import_id, group_id=None):
        ...
        if response.status_code == 200:
            ...
            raise HTTPError(
                response, f"Get import failed with status code: {response.json()}"
            )
    ...
    def get_imports(self, group_id=None):
        ...
        if response.status_code == 200:
            ...
            raise HTTPError(
                response, f"Get imports failed with status code: {response.json()}"
            )
