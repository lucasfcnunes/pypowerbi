...
class Features:
    ...
    def automatically_push_app_to_end_user(self):
        return self.get_available_features(
            feature_name="automaticallyPushAppToEndUsers"
        )

    @property
    def publish_app_to_entire_organization(self):
        return self.get_available_features(
            feature_name="publishAppToEntireOrganization"
        )
    ...
    def features_from_get_available_features_response(response):
        ...
        if "features" in response_dict:
            ...
            for feature in response_dict["features"]:
                additional_info = (
                    feature[Feature.additional_info_key]
                    if Feature.additional_info_key in feature
                    else None
                )
            ...
            additional_info = (
                response_dict[Feature.additional_info_key]
                if Feature.additional_info_key in response_dict
                else None
            )
            return Feature(
